...
* __Cases:__ Choose how to sort the cases, specify the maximum number of cases to display, group cases according to selected variables, and adjust the visible characters of the case labels
...
The row with KRAS cases will move below NRAS.
...
<!-- Click KRAS again to showcase the edit menu.

Click on `Replace` as shown above to replace KRAS gene variable with `Primary site` as shown below. The chart updates with the first row as `Primary site` thereby replacing KRAS gene variable as shown below. User may choose to sort samples by clicking the `Primary site` label.
...
This updates the chart. User may choose to add back KRAS through the gene panel.

Click on `Replace` as shown above to replace KRAS gene variable with `Primary site` as shown below.

Moreover, a user has the option to add rows for gene expression data. Click on `KRAS` gene label, click on the option `Gene expression` and search for a gene of interest as shown.
